"""The batch c2 quote-summary corpus coverage gate: earnings-family modules.

Every quote-summary capture's payload for each of the six earnings-family
batch c2 modules (``earnings``, ``earningsGaap``, ``earningsNonGaap``,
``earningsHistory``, ``earningsTrend``, ``earningsCallTranscripts``) must
...
``yoghurt.models.summary_earnings``. See
``tests/models/test_summary_financials_corpus.py`` for the remaining four
batch c2 (financial-snapshot) modules.
"""
...
_MODULE_MODELS: dict[str, type[YahooModel]] = {
    ...
    "earningsGaap": EarningsModule,
    "earningsNonGaap": EarningsModule,
...
def test_module_payload_validates_with_no_extra_fields(
    ...
    ``EarningsChartQuarter``, ``FinancialsChartYear``/``.Quarter``,
    ``EarningsHistoryEntry``, and ``EarningsTrendEntry`` (with its own
    nested ``EarningsEstimate``/``RevenueEstimate``/``EpsTrend``/
    ``EpsRevisions`` blocks) must all stay extras-free too, not just the
    top-level module model. This is also the gate that proves the ``{}``
    -means-``None`` unwrap extension (see ``yoghurt.models._base``) does
    not leave stray extras behind on the corpus's no-analyst-coverage
    examples (``BAC-PL``, ``7203.T``).
    """
