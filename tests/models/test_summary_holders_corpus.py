"""The batch c4 quote-summary corpus coverage gate: holder/ownership modules.

Every quote-summary capture's payload for each of the seven holder/ownership
batch c4 modules (``insiderHolders``, ``insiderTransactions``,
``institutionOwnership``, ``fundOwnership``, ``majorDirectHolders``,
``majorHoldersBreakdown``, ``netSharePurchaseActivity``) must validate
against its model with nothing landing on ``model_extra`` anywhere in the
model tree — the same drift alarm the earlier corpus gates enforce. This
file also pins each module's required-field set to its corpus-measured
universal keys (via ``tools.fields_report.quote_summary_module_records``),
proves the ``institutionOwnership``/``fundOwnership`` shared row-model
reuse, and enforces alphabetical field declaration order for every model
added in ``yoghurt.models.summary_holders``. See
``tests/models/test_summary_funds_corpus.py`` for the remaining three batch
c4 (fund-internals) modules.
"""
...
_EXPECTED_RECORD_COUNTS: dict[str, int] = {
    "insiderHolders": 9,
    "insiderTransactions": 7,
    "institutionOwnership": 9,
    "fundOwnership": 9,
    "majorDirectHolders": 9,
    "majorHoldersBreakdown": 9,
    "netSharePurchaseActivity": 9,
}

_EXPECTED_REQUIRED_FIELD_COUNTS: dict[str, int] = {
    "insiderHolders": 2,
    "insiderTransactions": 2,
    "institutionOwnership": 2,
    "fundOwnership": 2,
    "majorDirectHolders": 2,
    "majorHoldersBreakdown": 5,
    "netSharePurchaseActivity": 10,
}
...
_ROW_MODELS: dict[str, type[YahooModel]] = {
    ...
    "institutionOwnership": OwnershipEntry,
    "fundOwnership": OwnershipEntry,
}

_EXPECTED_ROW_REQUIRED_FIELD_COUNTS: dict[str, int] = {
    "insiderHolders": 6,
    "insiderTransactions": 9,
    "institutionOwnership": 7,
    "fundOwnership": 7,
}
...
def test_module_payload_validates_with_no_extra_fields(
    ...
    ``InsiderHolder``, ``InsiderTransaction``, and ``OwnershipEntry`` must
    all stay extras-free too, not just the top-level module model.
    """
...
def test_row_required_field_set_matches_corpus_universal_keys(module: str) -> None:
    """Each module's row model's required fields match corpus-measured keys.

    Unlike the module-level check above, this walks every individual row
    (not the module wrapper) across every capture that carries this
    module, pooling rows when the row model is shared
    (``institutionOwnership``/``fundOwnership`` both back ``OwnershipEntry``).
    """
...
def test_institution_and_fund_ownership_share_one_row_model() -> None:
    """institutionOwnership/fundOwnership reuse the identical OwnershipEntry.

    Per the plan's one-concept rule: these two modules share one wire
    shape end to end, so they share one model class rather than two
    near-identical models.
    """

    assert _ROW_MODELS["institutionOwnership"] is OwnershipEntry
    assert _ROW_MODELS["fundOwnership"] is OwnershipEntry


def test_major_direct_holders_is_always_empty_in_this_corpus() -> None:
    """majorDirectHolders.holders is an empty list on every corpus capture.
    ...
    that carries this module shows the same empty placeholder.
    """

    for record in quote_summary_module_records("majorDirectHolders"):
        instance = MajorDirectHolders.model_validate(dict(record))
        assert instance.holders == []
