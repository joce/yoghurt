...
- Applicability lines state restrictions only: a field observed universally across its model's corpus coverage carries no "Observed on:" line at all (absence means universal). A field restricted to a strict subset ends with exactly one of three forms: "Observed on: <types> <endpoint-noun>." / "Not observed in the corpus; known from prior use on <types> <endpoint-noun>." / "Observed only as empty lists in the corpus." The endpoint noun (quotes / charts / contracts / chains / …) is fixed per model module and stated in that module's docstring. The corpus capture date lives once in the module docstring.
